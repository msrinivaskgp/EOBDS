...
with open('C:\\Users\\M.S\\Documents\\CTRF-main\\CTRF-main\\CTRF\\CTRF\\CTRF\\Output/test5.pickle', 'rb') as file:
...
class bds_Func:
    def predict_bds(dt, bf, winetest):
        ...
        #print(node_counts)
        return acc3, arg2, count99, count111, count_s1,count_d1,count_c1,time1
    ...
    def predict_obds(dt, bf, winetest):
        ...
        for v in range(int(len(winetest))):
            ...
            for cc in range(0, Terms, 4):
                ...
                for d in range(0, trees, 1):
                    ...
                    # ✅ Convert to OBDD and gather statistics
                    #obdd_f1 = expr2bdd(f1)
                   # obdd_node_counts.append(len(obdd_f1.nodes))
                    #obdd_node_counts.append(len(list(obdd_f1.dfs_postorder())))
                    # Compute paths & avg path length
                    #paths = list(obdd_f1.satisfy_all())
                    #obdd_path_counts.append(len(paths))
                    ...
                    count_s2 =count_s2 + f1.size

                    count_d2 =count_d2 + f1.depth

                    count_c2 =count_c2 + f1.cardinality
        ...
        #print("Unsatisfiable OBDS trees:", sorted(unsat_trees))
        #print("Total unsatisfiable trees:", len(unsat_trees))
        print('-------------------------------------------')
        print('OBDS Accuracy:', correct / len(winetest))
        acc3 = correct / len(winetest)
        print(f"Avg Test Time (ms): {avg_test_time*1000:.4f}")
        time1 = avg_test_time * 1000
        ...
        return acc3, arg2, count99, count111, count_s2,count_d2,count_c2,time1
    ...
    def predict_eobds(dt, bf, winetest):
        ...
        #path_lengths = sum(obdd_avg_path_lengths)/len(obdd_avg_path_lengths)
        #node_counts = sum(obdd_node_counts) / len(obdd_node_counts)
        return acc3, arg2, count99, count111,count_s3,count_d3,count_c3,time3
